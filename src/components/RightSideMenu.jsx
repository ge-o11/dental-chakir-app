...
import {
  ...
  Globe, Sparkles, UserCircle, Menu,
} from 'lucide-react'
...
export default function RightSideMenu() {
  ...
  return (
    <>
      ...
      {/* Sidebar — always visible, collapses to icon strip when closed */}
      <aside
        className={`fixed top-0 right-0 z-50 h-full bg-white shadow-2xl
          flex flex-col overflow-hidden
          transition-[width] duration-300 ease-in-out
          ${state.menuOpen ? 'w-72' : 'w-[52px]'}`}
      >

        {/* ── MINI STRIP (always rendered, hidden via overflow when full) ── */}
        <div className={`absolute inset-0 flex flex-col items-center pt-[68px] pb-4 gap-1 px-1.5
          transition-opacity duration-200
          ${state.menuOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>

          {/* Expand button */}
          <button
            onClick={actions.toggleMenu}
            title={isRTL ? 'פתח תפריט' : 'Open menu'}
            className="w-9 h-9 rounded-xl flex items-center justify-center mb-1
              text-gray-400 hover:bg-brand-50 hover:text-brand-600 transition-all"
          >
            <Menu size={18} />
          </button>
          ...
          {MENU_ITEMS.map(({ key, icon: Icon, navKey }) => {
            const active = state.currentSection === key
            return (
              <button
                key={key}
                title={tr.nav[navKey]}
        ...
        {/* ── FULL SIDEBAR (slides in when menuOpen) ── */}
        <div className={`flex flex-col h-full transition-opacity duration-200
          ${state.menuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
              ...
              <div className="mt-3 rounded-xl overflow-hidden border border-teal-400/30"
                ...
                <a
                  ...
                  className="block w-full py-2 text-center text-xs font-bold text-teal-200
                    bg-teal-500/20 hover:bg-teal-500/35 transition-colors border-t border-teal-400/20"
                >
                  {isRTL ? '✨ הצטרף לקהילה' : '✨ Join the community'}
                </a>
              </div>
            ...
            <div className="flex gap-2">
              <a
                ...
                className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl
                  ...
                  hover:bg-brand-100 active:scale-95 transition-all duration-150 text-xs font-semibold"
              >
                <Globe size={13} />
              ...
              <a
                ...
                className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl
                  ...
                  hover:bg-blue-100 active:scale-95 transition-all duration-150 text-xs font-semibold"
              >
